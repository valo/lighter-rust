...
mod tests {
    ...
    fn test_nonce_increasing() {
        ...
        for _ in 0..50 {
            let nonce = manager.generate().unwrap();
            assert!(nonce > previous, "Nonce {} is not greater than previous {}", nonce, previous);
            previous = nonce;
        }
    }
        ...
        for _ in 0..10 {
            ...
            let handle = thread::spawn(move || {
                for _ in 0..10 {
                    ...
                    assert!(nonces_guard.insert(nonce), "Duplicate nonce in concurrent generation: {}", nonce);
                }
            });
    ...
    fn test_nonce_timestamp_format() {
        ...
        assert!(nonce > 1_000_000_000_000, "Nonce should be at least 13 digits");
